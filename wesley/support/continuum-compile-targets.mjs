...
function buildEchoGeneratedFiles(ir) {
  ...
  const unionTypes = (ir.types ?? []).filter((type) => type.kind === 'UNION');
  return [
    {
      path: 'schemas.generated.ts',
      content: buildEchoSchemasTypeScript({ objectTypes, enumTypes, unionTypes })
    },
    {
...
function buildEchoSchemasTypeScript({ objectTypes, enumTypes, unionTypes }) {
  const knownTypes = new Set([
    ...objectTypes.map((type) => type.name),
    ...enumTypes.map((type) => type.name),
    ...unionTypes.map((type) => type.name)
  ]);
  const lines = [
  ...
  for (const unionType of unionTypes) {
    lines.push(
      `export type ${unionType.name} = ${unionType.variants.join(' | ')};`,
      ''
    );
  }
  ...
  for (const type of objectTypes) {
    ...
    for (const field of type.fields) {
      lines.push(`  ${field.name}${field.required ? '' : '?'}: ${fieldTypeScriptType(field, knownTypes)};`);
    }
    lines.push('}', '');
  }
...
function fieldTypeScriptType(field, knownTypes) {
  const base = scalarTypeScriptType(field.type, knownTypes);
  return field.list ? `readonly ${base}[]` : base;
}

function scalarTypeScriptType(type, knownTypes) {
  switch (type) {
  case 'Int':
  case 'Float':
    return 'number';
  ...
  case 'Boolean':
    return 'boolean';
  default:
    return knownTypes.has(type) ? type : 'string';
  }
}
