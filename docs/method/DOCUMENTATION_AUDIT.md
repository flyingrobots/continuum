...
### Still open
- **Architecture drift (issue #47):** `continuum/wesley/` targets a removed JS-era Wesley (imports `@wesley/core` / `packages/wesley-host-node/bin/wesley.mjs`), but the real Wesley is now a **Rust** Cargo workspace with a native `wesley-cli`. The demo manifest, `wesley/README.md`, and `apps/warp` all reference the dead Node layout. This supersedes the earlier "vendor `@wesley/core` in CI" idea — there is no JS `@wesley/core` to vendor, and the receipt/settlement `fixture-witnessed` evidence is not runnable against current Wesley. Needs an architecture decision (retarget at `wesley-cli`, or remove the module) before the dependent docs/manifests are "corrected" to a layout that is itself dead.
