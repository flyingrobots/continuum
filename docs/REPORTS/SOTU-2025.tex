...
\textbf{The Resolution:} We will perform a ``Brain Transplant.'' The Rust Kernel (`echo`) will become the authoritative core of the JITOS Monorepo. The JavaScript codebase will be demoted to a ``Shell''—a pure View/IO layer that renders the state emitted by the Rust/WASM kernel. The Go Planner will be retired and ported to Rust to eliminate serialization overhead.
...
\section{Genealogy & Extraction}

JITOS is not a clean-slate invention; it is the synthesis of five years of research across multiple prototypes. We explicitly acknowledge and integrate these lineages:

\begin{itemize}
    \item \textbf{shiplog (Git-Backed Audit):} The \textit{Trust Model} for `jitos-provenance`. We adopt the concept of "Anchors" and the `.shiplog/trust.json` configuration for defining Agent Identity roots.
    \item \textbf{gatos (Git Operating Surface):} The \textit{Data Plane} architecture. We adopt "Opaque Pointers" (Wormholes) for heavy data and the "Ledger Kernel" concept. The Lua sandbox in `gatos-policy` is the direct ancestor of our Rhai `jitos-policy`.
    \item \textbf{wesley (Agent Framework):} The \textit{Application Layer}. We adopt "Contracts" and "Directives" as the semantic basis for SLAPS and HTN Methods. The `wesley-host-*` adapters inform our `jitos-wasm` bridge design.
    \item \textbf{libgitledger (C Ledger):} The \textit{Compliance} architecture. We adopt the "Compliance Suite" pattern: the Log is only valid if it passes a rigorous, independent audit suite.
    \item \textbf{git-mind (C Knowledge Graph):} The \textit{Attribution Domain}. We ensure `jitos-graph` supports edge-level attribution, allowing agents to dispute links within a shared graph.
\end{itemize}
...
\begin{enumerate}
    ...
    \item **Remote:** WebSockets carrying serialized SLAPS (Binary).
\end{enumerate}
