Section 7 — Temporal Semantics: WAL & Replay
...
👉 7. Section 7: Temporal Semantics

Let’s begin.

---

JITOS Architecture Document — Section 7
...
“Time” in JITOS is the existence of WAL entries
and their deterministic application into the RMG.

### 7.1.b The Echo Principle: Time as Ordered Echoes

The JITOS kernel treats time not as a wall-clock value, but as **an ordered series of echoes**.

Each WAL entry is:

- an echo of a prior state transition,
- a new tick in logical time,
- a durable imprint of a moment when reality changed.

Each collapse event:

- takes a speculative SWS,
- resolves it against the current truth,
- and adds a new echo to the universe in the form of a snapshot node.

The Echo Principle can be summarized as:

> **Every moment of computation leaves an echo in the substrate.  
> Temporal semantics are the ordering of those echoes.**

This is why:

- replay is possible (we can retrace echoes),  
- determinism is enforced (echoes are strictly ordered),  
- concurrency is safe (collapse events are serialized),  
- provenance is rich (semantic echoes are attached as graphs),  
- federation works (foreign universes are introduced as structured echoes).

In JITOS, the **ECHO layer**—whether a scheduler, a WAL interpreter, or a higher-level temporal arbitrator—is responsible for:

- accepting event intents,
- deciding their order,
- and committing them to the universe as new echoes.

It is intentionally left open whether one interprets ECHO as:

- *Event ordering along a causal horizon*, or  
- *the historical accumulation of all execution*.

Both interpretations are valid at different scales of the RMG.  
That ambiguity is not an accident—it reflects the multi-scale nature of the system itself.
...
- same input WAL → same output RMG
- across machines
- across time
...
This preserves the kernel’s temporal invariants.
