...
| `runtime-boundary-family` | `0.1.0` | [`schemas/continuum-runtime-boundary-family.graphql`](../schemas/continuum-runtime-boundary-family.graphql) | `IntentEnvelope`, `TickResult`, `ObserverPlan`, `ObservationRequest`, `ReadingEnvelope`, `SuffixShell`, `ImportOutcome` | `authored` | `runtime-open`, `interop-open` | Echo, `git-warp`, Wesley, `warp-ttd`, app/tool hosts | Authored schema and design packet `0028` | Add Wesley profile, then prove witnessed suffix exchange/admission between sibling runtimes. |
...
| `SuffixShell` | Witnessed causal suffix export/import shell. | Echo, `git-warp`, or another sibling runtime. | Sibling runtimes and Wesley witness tooling. | Export fixture with base and target frontier digests. |
| `ImportOutcome` | Distributed suffix admission result. | Target sibling runtime. | Source sibling runtime, `warp-ttd`, app hosts. | Admission witness linking outcome to `SuffixShell`. |
