...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `receipt-family` | `0.1.0` | [`schemas/continuum-receipt-family.graphql`](../schemas/continuum-receipt-family.graphql) | `Receipt`, `DeliveryObservation`, `Capability`, `Witness` | `authored` | `runtime-open` | Echo, `git-warp`, `warp-ttd`, app/tool repos through generated artifacts | Authored schema; the prior Wesley witness suite was removed with `continuum/wesley` (issue #47) | Re-establish a compile/witness profile on the Rust `wesley-cli` target, then prove live sibling-runtime receipt publication. |
| `settlement-family` | `0.1.0` | [`schemas/continuum-settlement-family.graphql`](../schemas/continuum-settlement-family.graphql) | `SettlementRequest`, `SettlementDelta`, `ImportCandidate`, `ConflictArtifact`, `SettlementDecision`, `SettlementPlan`, `SettlementResult` | `authored` | `runtime-open` | Echo, `git-warp`, `warp-ttd`, app/tool repos through generated artifacts | Authored schema; the prior Wesley witness suite was removed with `continuum/wesley` (issue #47) | Re-establish a compile/witness profile on the Rust `wesley-cli` target, then prove live settlement values from sibling-runtime suffix/import flows. |
...
| `runtime-boundary-family` | `0.1.0` | [`schemas/continuum-runtime-boundary-family.graphql`](../schemas/continuum-runtime-boundary-family.graphql) | `IntentEnvelope`, `TickResult`, `ObserverPlan`, `ObservationRequest`, `ReadingEnvelope`, `ContinuumEvidenceStatus`, `ContinuumNativeEvidence`, `TranslatedSubstrateEvidence`, `WitnessedSuffixShell`, `CausalSuffixBundle`, `ImportOutcome` | `authored` | `runtime-open`, `interop-open` | Echo, `git-warp`, Wesley, `warp-ttd`, app/tool hosts | Authored schema and design packets `0027`/`0028` | Add Wesley profile, then prove witnessed suffix exchange/admission between sibling runtimes and compatibility evidence status in consumers. |
...
| `ReadingEnvelope` | Observer-relative reading result carrying payload identity plus native-or-translated evidence status. | Echo, `git-warp`, compatibility adapters, or another sibling runtime. | App hosts, `warp-ttd`, sibling runtimes. | Witness that reading links request, plan, lane, and honest evidence status. |
| `ContinuumEvidenceStatus` | Union that separates native Continuum evidence from translated substrate evidence. | Continuum-native runtimes or compatibility adapters. | App hosts, Graft, `warp-ttd`, sibling runtimes. | Generated artifacts and fixtures proving consumers do not equate compatible shape with native witnesshood. |
| `ContinuumNativeEvidence` | Evidence branch for actual Continuum-producing runtime witnesshood. | Echo or another runtime that publishes native Continuum boundary artifacts. | App hosts, Graft, `warp-ttd`, sibling runtimes. | Fixture-backed native witness publication. |
| `TranslatedSubstrateEvidence` | Evidence branch for compatibility evidence translated from a non-native substrate. | `git-warp` adapters, Graft adapters, or other compatibility layers. | App hosts, Graft, `warp-ttd`, sibling runtimes. | Fixture proving translated evidence cannot claim a native Continuum witness. |
...
| `git-warp` | Sibling runtime implementation that may emit and consume conforming values once native support exists; until then, committed-history facts are translated substrate evidence, not Continuum-native witnesses. | The durable half of Echo or a source of fake Continuum witnesses. |
| `warp-ttd` | Debugger, operator surface, and generated-artifact consumer. | A hand-normalized substitute for shared contracts. |
| Graft | Planned structural observer and review engine; expected consumer of runtime-boundary observer nouns such as `ObserverPlan`, `ObservationRequest`, `ReadingEnvelope`, and `ContinuumEvidenceStatus`; producer of app-local code-aware structural reading payloads until promotion is warranted. Graft is not a native Continuum publisher unless it is relaying native runtime evidence. | A runtime implementation, debugger product, shadow Continuum semantic owner, permanent host-normalization layer, or laundering path for non-native evidence. |
...
| --- | --- | --- |
| Wesley can compile and witness the receipt family. | `authored` (witness suite removed with `continuum/wesley`, issue #47) | Re-establish the compile/witness profile on the Rust `wesley-cli` target. |
| Wesley can compile and witness the settlement family. | `authored` (witness suite removed with `continuum/wesley`, issue #47) | Re-establish the compile/witness profile on the Rust `wesley-cli` target. |
